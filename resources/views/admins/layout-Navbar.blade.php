  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
        ...
        @if (Auth::check() && Auth::user()->permission['manage_newsfeed'] ?? false)
        <li class="nav-item">
          <a class="nav-link text-dark" aria-current="page" href="{{ route('type_list') }}"
            onmouseover="this.classList.replace('text-dark', 'text-warning')"
            onmouseout="this.classList.replace('text-warning', 'text-dark')">
            <i class="fas fa-newspaper"></i> จัดการกิจกรรม
          </a>
        </li>
        @endif
      ...
      @if (Auth::check() && Auth::user()->permission['manage_newsfeed'] ?? false)
      <li class="nav-item">
        <a class="nav-link text-dark" aria-current="page" href="{{ route('provinceactivityList') }}"
          onmouseover="this.classList.replace('text-dark', 'text-warning')"
          onmouseout="this.classList.replace('text-warning', 'text-dark')">
          <i class="fas fa-newspaper"></i> จัดการจังหวัดและศูนย์บริการ
        </a>
      </li>
      @endif
    </ul>
