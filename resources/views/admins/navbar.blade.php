    ...
    <ul class="navbar-nav">
        ...
        @if (Auth::check() && Auth::user()->permission['manage_users'] ?? false)
            <li class="nav-item d-none d-sm-inline-block">
                <a href="{{ route('users.list') }}" class="nav-link text-dark"><i class="fas fa-users-cog"></i>
                    จัดการผู้ใช้</a>
            </li>
        @endif
        ...
        @if (Auth::check() && Auth::user()->permission['manage_newsfeed'] ?? false)
            <li class="nav-item d-none d-sm-inline-block">
                <a href="{{ route('listnewsfeed') }}" class="nav-link text-dark"><i class="fas fa-newspaper"></i>
                    จัดการเอกสารข่าว</a>
            </li>
        @endif
        ...
        @if (Auth::check() && Auth::user()->permission['manage_dashboard'] ?? false)
            <li class="nav-item d-none d-sm-inline-block">
                <a href="{{ route('importdata') }}" class="nav-link text-dark"><i class="fas fa-chart-line"></i>
                    จัดการแดชบอร์ด</a>
            </li>
        @endif
            ...
            @auth
                <a class="nav-link d-flex justify-content-center align-items-center" data-toggle="dropdown" href="#">
                    @if (Auth::user()->profile_image)
                        <img src="{{ Auth::user()->profile_image }}" alt="User Avatar" class="img-size-50 img-circle mr-2">
                    @else
