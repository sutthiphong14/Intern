        ...
        <tbody>
            @foreach ($data as $row)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('servicecenteractivityedit', $row->center_id) }}" class="btn btn-warning btn-sm">Edit</a>
                         <form action="{{ route('servicecenteractivitydelete', $row->center_id) }}" method="POST" style="display: inline-block;">
                            @csrf
                            @method('DELETE')
