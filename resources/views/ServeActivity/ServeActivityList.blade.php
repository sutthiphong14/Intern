            ...
            @foreach ($data as $row)
                <tr>
                    ...
                    <td>
                        ...
                        <form action="{{ route('service_delete', $row->service_id) }}" method="POST" style="display: inline-block;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure?')">Delete</button>
                        </form>
                        <a href="{{route('promotion_list',$row->service_id)}}" class="btn btn-success btn-sm">เพิ่มโปรโมชั่น</a>
                    </td>
                </tr>
            @endforeach
        </tbody>
